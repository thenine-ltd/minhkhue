 ...
 *
 * @package           Woocommerce Add to cart Ajax for variable products
 * @author            Rishi Mehta
 * @copyright         2014-2024 Rishi Mehta
 * @license           GPL-2.0+
 *
 * @wordpress-plugin
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || ( get_site_option('active_sitewide_plugins') && array_key_exists( 'woocommerce/woocommerce.php', get_site_option('active_sitewide_plugins') ) ) ) {
	...
	**/
	...
	function wc_ajax_add_to_cart_variable_add_section( $sections ) {
		...
		$sections['wc_ajax_add_to_cart_variable'] = __( 'WC Ajax for Variable Products', 'woocommerce-ajax-add-to-cart-variable-products' );
		return $sections;
	...
	function wc_ajax_add_to_cart_variable_all_settings( $settings, $current_section ) {
		...
		if ( $current_section == 'wc_ajax_add_to_cart_variable' ) {
			...
			$settings_slider[] = array( 'name' => __( 'WC Ajax for Variable Products Settings', 'woocommerce-ajax-add-to-cart-variable-products' ), 'type' => 'title', 'desc' => __( 'The following options are used to configure WC Ajax for Variable Products', 'woocommerce-ajax-add-to-cart-variable-products' ), 'id' => 'wc_ajax_add_to_cart_variable' );
	 
			// Add first checkbox option
			$settings_slider[] = array(
	 
				'name'     => __( 'Add Selection option to Category Page', 'woocommerce-ajax-add-to-cart-variable-products' ),
				'desc_tip' => __( 'This will automatically insert variable selection options on product Category Archive Page', 'woocommerce-ajax-add-to-cart-variable-products' ),
				'id'       => 'wc_ajax_add_to_cart_variable_category_page',
				'type'     => 'checkbox',
				'css'      => 'min-width:300px;',
				'desc'     => __( 'Enable Varition select option on Category Archive page', 'woocommerce-ajax-add-to-cart-variable-products' ),
